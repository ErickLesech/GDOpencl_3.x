...
opts.Add(PathVariable("target_name", "The library name.", "libproject", PathVariable.PathAccept))
...
env.Append(
    LIBS=[
        env.File(os.path.join("godot-cpp/bin", "libgodot-cpp.%s.%s.64%s" % (platform, env["target"], env["LIBSUFFIX"]))),env.File(os.path.join("godot-cpp/bin", "OpenCL.lib"))
    ]
)

#ajout de librairie Opencl

#env.Append(LIBS=[env.File(os.path.join("godot-cpp/bin", "OpenCL.lib"))])
